...
**Environments that preserve detached background processes:**
```bash
...
```bash
# Run in foreground mode and use your shell tool's background execution
# mechanism so the server survives across turns.
scripts/start-server.sh --project-dir /path/to/project --foreground
```
When calling this via a shell tool, use that tool's background execution option. Then read `$STATE_DIR/server-info` on the next turn to get the URL and port.
